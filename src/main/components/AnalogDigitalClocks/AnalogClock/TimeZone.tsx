...
type TimeZonePropsType = {
  ...
  setTimezone: (zone: Array<ZoneItemType>) => void;
};
...
export const TimeZone: React.FC<TimeZonePropsType> = ({
  timezone,
  setTimezone,
}) => {
  const options: Array<ZoneItemType> = [
    {
      value: "-11.0",
  ...
  const selectTimeZone = (e: ChangeEvent<any>) => {
    let findZone = options.filter((zone) => zone.location === e.target.value);
    setTimezone(findZone);
  };

  const mappedOptions: any[] = options
    ? options.map((el, ind) => (
        <MenuItem key={ind} value={el.location}>
          {el.label}
        </MenuItem>
      ))
